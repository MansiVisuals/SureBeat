...
local server_url = "https://activate.mansivisuals.com"  -- Updated server URL
...
local surebeat_binary = script_path .. "SureBeat.exe" -- Add .exe extension for Windows
...
local CURRENT_VERSION = "v3.0.0"

-- Create temp directory at startup using Windows command style
os.execute('if not exist "' .. temp_folder_path .. '" mkdir "' .. temp_folder_path .. '"')

-- Function to execute a cURL command and return the response
function execute_curl(url, data, use_api_key)
    -- Properly escape double quotes in JSON payload for Windows as in SureBeat.lua
    local command
    if use_api_key then
        local escaped_data = data:gsub('"', '\\"')
        command = string.format('curl -s -X POST "%s" -H "Content-Type: application/json" -H "x-api-key: %s" -d "%s"', url, api_key, escaped_data)
    else
        local escaped_data = data:gsub('"', '\\"')
        command = string.format('curl -s -X POST "%s" -H "Content-Type: application/json" -d "%s"', url, escaped_data)
    end
    local handle = io.popen(command)
    if not handle then
        print("[SureBeat] ERROR: Failed to execute cURL command.")
        return ""
    end
...
function validate_with_server(license_key)
    ...
    local response = execute_curl(server_url .. endpoint, request_body, true)
...
function activate_license(license_key, mac_uid)
    ...
    local response = execute_curl(url, request_body, true)
...
function save_license_key(license_key)
    ...
    if file then
        ...
        print("[SureBeat] ERROR: Unable to save the license key.")
    end
end

function get_mac_uid()
    -- Windows version of MD5 hashing using PowerShell
    local function md5_hash(str)
        -- Escape special characters for PowerShell command
        local escaped_str = str:gsub("'", "''")
        
        local cmd = string.format('powershell -Command "[System.BitConverter]::ToString([System.Security.Cryptography.MD5]::Create().ComputeHash([System.Text.Encoding]::UTF8.GetBytes(\'%s\'))).Replace(\'-\',\'\')"', escaped_str)
        local handle = io.popen(cmd)
        local result = handle:read("*a"):gsub("%s+", ""):lower()
        handle:close()
        return result
    end

    -- Use Get-CimInstance to get the motherboard serial number
    local handle = io.popen('powershell -Command "Get-CimInstance -Class Win32_BaseBoard | Format-Table -HideTableHeaders SerialNumber"')
    if not handle then
        return "unknown-windows-serial"
    end
    local uuid = handle:read("*a")
    handle:close()
    
    -- Clean and validate the UUID
    uuid = uuid and uuid:gsub("%s+", "")
    
    -- If primary method fails, try alternative CIM commands (no WMIC)
    if not uuid or uuid == "" then
        -- Try BIOS serial number instead
        handle = io.popen('powershell -Command "Get-CimInstance -Class Win32_BIOS | Format-Table -HideTableHeaders SerialNumber"')
        local bios = handle:read("*a"):gsub("%s+", "")
        handle:close()
        
        -- Try computer system name
        handle = io.popen('powershell -Command "Get-CimInstance -Class Win32_ComputerSystem | Format-Table -HideTableHeaders Name"')
        local name = handle:read("*a"):gsub("%s+", "")
        handle:close()
        
        uuid = (name or "PC") .. "-" .. (bios or tostring(os.time()))
    end
    
    -- Generate hash from the UUID using MD5
    local hash = md5_hash(uuid)
    ...
    local mac_address = table.concat(macParts, ":")
    return mac_address
end
...
function open_license_manager(initial_status, trial_active, trial_remaining)
    ...
    -- Update Buy Button to use Windows URL opener as in SureBeat.lua
    license_win.On.Buy.Clicked = function() 
        os.execute('start "" "https://ko-fi.com/s/2455c69d4d"') -- Windows URL opener
    end
...
function open_update_window(disp, main_win, advanced_win)
    ...
    function fetch_versions()
        -- Keep the Windows-specific endpoint (this is needed behind the scenes)
        local url = update_server_url .. "/windows/versions"
        print("[SureBeat] Fetching available versions from: " .. url)
        ...
        if result and result ~= "" then
            ...
            if versions_data and versions_data.versions then
                ...
                if #versions_data.versions > 0 then
                    items.UpdateInfo:SetText("Select a version of SureBeat to install.\nCurrent version: " .. CURRENT_VERSION)
                    version_dropdown.CurrentIndex = 0
                else
                    items.UpdateInfo:SetText("No versions available.")
                end
            else
    ...
    function download_version(version)
        -- Keep the Windows-specific endpoint (needed for functionality)
        local url = update_server_url .. "/windows/download/" .. version
        ...
        if success then
            ...
            if file then
                file:close()
                items.UpdateInfo:SetText("SureBeat version " .. version .. " installed successfully.\n" .. 
    ...
    function update_win.On.RestartSureBeat.Clicked()
        ...
        -- Path to SureBeat_main.luac for Windows
        local compiledScriptPath = script_path .. "SureBeat_main.luac"
...
function detect_beat_and_tempo(audio_path)
    ...
    local command = string.format('cmd /c ""%s" "%s" 2>&1"', surebeat_binary, audio_path)
    print("[SureBeat] Running command:", command)

    local handle = io.popen(command, "r")
    if not handle then
        print("[SureBeat] ERROR: Could not execute SureBeat. Check if the file exists.")
        print("[SureBeat] HINT: Make sure SureBeat.exe is correctly installed in: " .. script_path)
        return {}, {}
    end
    ...
    if not success then
        ...
        print("  - The SureBeat file is not executable or missing (check permissions).")
        return {}, {}
    end
