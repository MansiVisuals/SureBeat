...
app.post("/activate-license", (req, res) => {
  ...
  if (!licenseKey || !macUid) {
    return res
      .status(400)
      .json({ status: "error", message: "License key and MAC UID are required." });
  }

  if (!isValidLicenseKey(licenseKey) || !isValidMacUid(macUid)) {
    return res.status(400).json({
      status: "error",
      message: "Invalid license key or MAC UID format.",
    });
  }
    ...
    db.get("SELECT * FROM Licenses WHERE license_key = ?", [encrypt(licenseKey)], (err, row) => {
      ...
      if (row.mac_uid) {
        if (row.mac_uid === encryptedMacUid) {
          ...
          return res.status(400).json({
            status: "error",
            message: "License is already activated on another device. Contact support for assistance.",
          });
        }
      }
  ...
  if (licenseKey === "trial") {
    ...
    db.get("SELECT * FROM Licenses WHERE license_key = ?", [encrypt(licenseKey)], (err, row) => {
      ...
      if (row.mac_uid && row.mac_uid !== encryptedMacUid) {
        return res.status(400).json({
          status: "error",
          message: "License is activated on another device. Contact support for assistance.",
        });
      }
